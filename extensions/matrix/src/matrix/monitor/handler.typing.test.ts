...
describe("Matrix Handler Typing and Streaming Integration", () => {
  afterEach(() => {
    vi.useRealTimers();
    mockOnReplyStart.mockClear();
  });

  it("re-signals typing after each block delivery (with 1s delay)", async () => {
    ...
    // Switch to fake timers only after handler completes, so its internal
    // Promise chain is not frozen during setup.
    vi.useFakeTimers();

    // Trigger block delivery. enqueueSend inside sendMessageMatrix has a 150ms gap
    // timer; advance past it so the delivery can complete, then the 1s re-trigger
    // setTimeout gets scheduled.
    const deliverPromise = deliverCallback?.({ text: "block" });
    await vi.advanceTimersByTimeAsync(200); // unblock the 150ms enqueueSend gap
    await deliverPromise;

    // Re-trigger is delayed by 1s — should NOT have fired yet
    expect(mockOnReplyStart).not.toHaveBeenCalled();

    // Advance past the 1s re-trigger delay
